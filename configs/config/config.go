...
const (
	defaultConfigFilename = "multivac.conf"
	defaultDataDirname    = "data"
	defaultLogLevel       = "info"
	defaultLogDirname     = "logs"
	defaultLogFilename    = "mtv.log"
	defaultMaxPeers       = 64
	defaultMinShardPeers  = 8
	defaultBanDuration    = time.Hour * 24
	defaultBanThreshold   = 100
	// DefaultConnectTimeout defines the default time of timeout.
	...
	localConfigFilename          = "multivac_dev.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
	defaultVoteThreshold         = 100
	defaultSharded               = true
	defaultLeaderRate            = 4
	defaultTimeForPeer           = 30
	defaultRestart               = false
	defaultIsOneOfFirstMiners    = false
	defaultPk                    = ""
	defaultSk                    = ""
)
...
var (
	defaultHomeDir    = btcutil.AppDataDir("multivac", false)
...
type Config struct {
	...
	MonitorListeners     []string      //`long:"monitorlisten" description:"Add an interface/port to listen monitoring scrap"`
	MaxPeers             int           //`long:"maxpeers" description:"Max number of inbound and outbound peers"`
	MinShardPeers        int32         //`long:"minshardpeers" description:"Min number of shard peers"`
	...
	RunAsDNSSeed         bool          //`long:"runasdnsseed" description:"Run as DNS seeding, this will disable load peers and dns seeding for current node"`
	EnableRelayMessage   bool          //`long:"enablerelay" description:"Enable relay message in network, default false"`
	...
	StorageNode          bool          //`long:"storagenode" description:"Run as a standalone storage node"`
	...
	VoteThreshold        int           //`long:"votethreshold" description:"The vote threshold for consensus algorithm"`
	DummySortitionor     bool          //`long:"dummysort" description:"Use dummy sortitionor which will always return true"`
	IntegrationTest      bool          //`long:"integtest" description:"Whether in integration test mode"`
	IsSharded            bool          //`long:"sharded" description:"Whether to enable sharding"`
	Shards               []uint32      //`long:"shard" description:"Used by storage node only, specifies which shard this storage node is going to serve. Ignored by other type of nodes."`
	ExpNumShard          int           //`long:"expnumshard" description:"Expected number of shards a node can hold. If unspecified, it will be the maximum number of shards."`
	LeaderRate           int           //`long:"leaderrate" description:"Of leaderRate number of Leaders, only one is expected to be leader."`
	TimeForPeerDiscovery int           //`long:"timeforpeerdiscovery" description:"Wait a while for peer discovery and sync."`
	Restart              bool          //`long:"restart" description:"whether restart mode ."`
	IsOneOfFirstMiners   bool          //`long:"isoneoffirstminers" description:"Whether the server is the first batch of miners."`
	RewardAddr           string        //`long:"rewardaddr" description:"Provide a reward address to make transactions."`
	Sk                   string `long:"sk" description:"the private for the miner."`
	UpdateWatchURL       string //`long:"updatewatchurl" description:"The URL for polling new version update."`
	UpdateIntervalSec    int    //`long:"updateintervalsec" description:"The polling interval for update in seconds."`
...
func parseAndSetDebugLevels(debugLevel string) error {
	...
	for _, logLevelPair := range strings.Split(debugLevel, ",") {
		...
		// Validate subsystem.
		if _, exists := logger.GetLogger(subsysID); !exists {
...
func getValidShards(shards []uint32) []uint32 {
	var validShards []uint32
	for _, id := range shards {
		if shard.IsValidShardID(id) {
			validShards = append(validShards, id)
		}
	}
	return validShards
}
...
func LoadConfig() ([]string, error) {
	...
	cfg := Config{
		...
		MinShardPeers:        defaultMinShardPeers,
		...
		IsSharded:            defaultSharded,
		ExpNumShard:          params.GenesisNumberOfShards,
		LeaderRate:           defaultLeaderRate,
		TimeForPeerDiscovery: defaultTimeForPeer,
		Restart:              defaultRestart,
		IsOneOfFirstMiners:   defaultIsOneOfFirstMiners,
		Pk:                   defaultPk,
		Sk:                   defaultSk,
		RPCPass:              "mtv",
		RPCUser:              "mtv",
	...
	if preCfg.MinShardPeers < defaultMinShardPeers {
		cfg.MinShardPeers = defaultMinShardPeers
	} else {
		cfg.MinShardPeers = preCfg.MinShardPeers
	}
	...
	if err != nil {
		...
		// NOTE: When running unit test by "go test xxx", args like `-test.testlogfile=xxxx`
		// may be added automatically, which will cause `unknown flag` error in parser.Parse().
		// So we have to ignore such parsing errors to make the unit test go through.
		//
		// return nil, err
	}
	...
	if cfg.DisableRPC {
		logger.BtcdLogger().Infof("RPC service is disabled")
	}
	...
	// Add supported shards for storage node.
	cfg.Shards = getValidShards(cfg.Shards)
	...
	if cfg.StorageNode || cfg.RunAsDNSSeed {
		cfg.MaxPeers = 1024
	}
	cfg.EnableRelayMessage = true
	cfg.DisableTLS = true
	//if cfg.Proxy != "" {
	...
	if configFileError != nil {
		logger.BtcdLogger().Warnf("%v", configFileError)
	}
...
// createDefaultConfig copies the file multivac_dev.conf to the given destination path,
...
func createDefaultConfigFile(destinationPath string) error {
	...
	localConfigPath := filepath.Join(path, localConfigFilename)
	...
	src, err := os.Open(localConfigPath)
	if err != nil {
		return err
	}
	defer src.Close()
...
// IsIPWhitelisted returns whether the IP address is included in the whitelisted
// networks and IPs.
func (cfg *Config) IsIPWhitelisted(addr net.Addr) bool {
	if len(cfg.whitelists) == 0 {
		return false
	}
	...
	if err != nil {
		logger.ServerLogger().Warnf("Unable to SplitHostPort on '%s': %v", addr, err)
		return false
	}
	ip := net.ParseIP(host)
	if ip == nil {
		logger.ServerLogger().Warnf("Unable to parse IP '%s'", addr)
		return false
	}
...
func GlobalConfig() *Config {
	// TODO(huangsz): log fatal or panic if globalCfg hasn't been loaded yet.
	return globalCfg
}
...
const (
	// MinerNode defines the role of MultiVAC.
	MinerNode = iota
	// StorageNode defines the role of MultiVAC.
	StorageNode
)

// GetNodeType returns the type of this MultiVAC node instance.
// Currently only supports miner type and storage node type.
func GetNodeType() NodeType {
	return ParseNodeType(globalCfg.StorageNode)
}

// ParseNodeType returns the NodeType from isStorageNode bool.
func ParseNodeType(isStorageNode bool) NodeType {
	if isStorageNode {
		return StorageNode
	}
	return MinerNode
