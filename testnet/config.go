...
var (
	btcdHomeDir           = btcutil.AppDataDir("multivac", false)
	btcctlHomeDir         = btcutil.AppDataDir("multivacctl", false)
	btcwalletHomeDir      = btcutil.AppDataDir("multivacwallet", false)
	defaultConfigFile     = filepath.Join(btcctlHomeDir, "multivacctl.conf")
...
func listCommands() {
	...
	fmt.Printf("the cmd method list is %+v", cmdMethods)
...
func createDefaultConfigFile(destinationPath string) error {
	...
	// Read multivac.conf from its default path
	btcdConfigPath := filepath.Join(btcdHomeDir, "multivac.conf")
	btcdConfigFile, err := os.Open(btcdConfigPath)

	if os.IsNotExist(err) {
		btcdConfigFile, err = os.Create(btcdConfigPath)
	}
	...
	_, err = dest.WriteString(fmt.Sprintf("rpcuser=%s\nrpcpass=%s", string(userSubmatches[1]), string(passSubmatches[1])))
	if err != nil {
		return fmt.Errorf("failed to write string,err:%v", err)
	}
	return nil
}
