...
func TestMessage(t *testing.T) {
	...
	testShardIndex := shard.Index(4)

	testByzAgreementValue := ByzAgreementValue{
	...
	msgBinaryBA := NewMessageBinaryBA(
		[]byte{},
		&SignedMsg{
			Pk:        []uint8{},
			Signature: []uint8{},
			Message: ConsensusMsg{
				CredentialWithBA: NewCredentialWithBA(testShardIndex, 2, 3, 4, testByzAgreementValue)}})
	msgLeaderProposalMessage := &LeaderProposalMessage{
	...
	msgLeaderVoteMessage := &LeaderVoteMessage{
		ShardIndex:   testShardIndex,
		InShardProof: []byte{},
	}
	msgGcMessage := &GCMessage{
		ShardIndex:   testShardIndex,
		Seed:         []byte{},
		InShardProof: []byte{},
	}
	...
	tests := []struct {
		...
		{msgBlock, msgBlock, pver, MainNet, 289},
		{msgTx, msgTx, pver, MainNet, 74},
		...
		{msgBinaryBA, msgBinaryBA, pver, MainNet, 83},
		{msgLeaderProposalMessage, msgLeaderProposalMessage, pver, MainNet, 64},
		{msgLeaderVoteMessage, msgLeaderVoteMessage, pver, MainNet, 30},
		{msgGcMessage, msgGcMessage, pver, MainNet, 31},
	}
