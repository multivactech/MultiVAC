...
var (
	...
	// defaultAddNewPeerSteps is the default number of new peer to additional add every time
	defaultAddNewPeerSteps = uint64(4)
)
		...
		case req := <-cm.requests:
			...
			case registerPending:
				...
				for _, c := range pending {
					if c.Addr != nil && connReq.Addr != nil && c.Addr.String() == connReq.Addr.String() {
						duplicate = true
						break
					}
				}
			...
			case handleConnected:
				...
				atomic.AddUint64(&cm.connectedCount, uint64(1))
			...
			case handleDisconnected:
				...
				// Subtract 1
				atomic.AddUint64(&cm.connectedCount, ^uint64(0))
...
func (cm *ConnManager) Connect(c *ConnReq) {
	...
	if atomic.LoadUint64(&c.id) == 0 {
		...
		// cancel the connection via the Remove method. This will judge if the address is duplicated
		valid := make(chan bool)
		select {
		case cm.requests <- registerPending{c, valid}:
		case <-cm.quit:
			return
		}
...
// AddNewConnections may start new connections when the number of peers in shards is not enough.
func (cm *ConnManager) AddNewConnections() {

	// Ignore request when still has pending connection request
	for atomic.LoadUint64(&cm.connectedCount) < atomic.LoadUint64(&cm.targetConnectedCount) {
		return
	}

	atomic.AddUint64(&cm.targetConnectedCount, defaultAddNewPeerSteps)

	for i := atomic.LoadUint64(&cm.connectedCount); i < atomic.LoadUint64(&cm.targetConnectedCount); i++ {
		go cm.NewConnReq()
	}
}
...
func (cm *ConnManager) Start() {
	...
	if atomic.LoadUint64(&cm.targetConnectedCount) < uint64(cm.cfg.TargetOutbound) {
		atomic.StoreUint64(&cm.targetConnectedCount, uint64(cm.cfg.TargetOutbound))
	}
