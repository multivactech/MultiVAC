...
import (
	...
	"github.com/multivactech/MultiVAC/interface/ipeer"
...
type MessageListeners struct {
	...
	// OnSendHeaders is invoked when a peer receives a send headers bitcoin
	// message.
	...
	OnRead func(bytesRead int, msg wire.Message, err error)
	...
	// HandleNewMsg handles a new incoming message.
...
type Config struct {
	...
	// Enable relay message. Whether forward incoming message to relay network.
	EnableRelayMessage bool
}
...
type Peer struct {
	...
	verAckReceived       bool
	witnessEnabled       bool
	relayRules           map[string]ipeer.MsgRelayRule
...
func (p *Peer) localVersionMsg() (*wire.MsgVersion, error) {
	...
	if err != nil {
		logger.PeerLogger().Debugf("failed to add user agent,err:%v", err)
	}
...
func (p *Peer) readMessage(encoding wire.MessageEncoding) (wire.Message, []byte, error) {
	...
	if p.cfg.Listeners.OnRead != nil {
		p.cfg.Listeners.OnRead(n, msg, err)
	}
	if err != nil {
		return nil, nil, err
	}
	...
	log.Debugf("%v", newLogClosure(func() string {
		...
		return fmt.Sprintf("Received %v%s from %s. Details: %v",
			msg.Command(), summary, p, msg)
	}))
...
func (p *Peer) writeMessage(msg wire.Message, enc wire.MessageEncoding) error {
	...
	log.Debugf("%v", newLogClosure(func() string {
		...
		return fmt.Sprintf("Sending %v%s to %s. Details: %v", msg.Command(),
			summary, p, msg)
	}))
	log.Tracef("%v", newLogClosure(func() string {
		return spew.Sdump(msg)
	...
	if p.cfg.Listeners.OnWrite != nil {
		p.cfg.Listeners.OnWrite(n, msg, err)
	}
	return err
}
...
out:
	for atomic.LoadInt32(&p.disconnect) == 0 {
		...
		default:
			if p.cfg.Listeners.HandleNewMsg != nil {
				go p.cfg.Listeners.HandleNewMsg(Reply(p.QueueMessage), msg, buf)
			}
		}
...
out:
	for {
		...
		case msg := <-p.sendQueue:
			if p.cfg.ChainParams.Net != wire.SimNet {
				log.Debugf("out message %v", msg.msg.Command())
			}
...
func (p *Peer) AssociateConnection(
	conn net.Conn,
	updatePeerInfoMessage wire.UpdatePeerInfoMessage) {
	// Already connected?
	...
	go func() {
		if err := p.start(updatePeerInfoMessage); err != nil {
...
func (p *Peer) start(updatePeerInfoMessage wire.UpdatePeerInfoMessage) error {
	...
	p.QueueMessage(&updatePeerInfoMessage, nil)
	return nil
}
...
// RegisterRelayRules register a extrenal reply rule of message to the peer.
func (p *Peer) RegisterRelayRules(messageType string, rule ipeer.MsgRelayRule) error {
	if _, ok := p.relayRules[messageType]; !ok {
		p.relayRules[messageType] = rule
		return nil
	}
	return errors.New("the rules already exist")
}

// RemoveRelayRules remove the rely rule for the specified message type.
func (p *Peer) RemoveRelayRules(messageType string) error {
	if _, ok := p.relayRules[messageType]; ok {
		delete(p.relayRules, messageType)
		return nil
	}
	return errors.New("Rules not found")
}

// applyRelyRules applies a rely rule to a given message,
// if no rules or execute faild it returns the original message.
func (p *Peer) applyRelayRules(msg wire.Message) (wire.Message, error) {
	if f, ok := p.relayRules[msg.Command()]; ok {
		rmsg, err := f.Filter(msg)
		if err != nil {
			return msg, err
		}
		return rmsg, nil
...
func newPeerBase(origCfg *Config, inbound bool) *Peer {
	...
	p := Peer{
		...
		relayRules:      make(map[string]ipeer.MsgRelayRule),
...
// Reply is used to replace *peer.Peer to reply some special message for the connected node.
type Reply func(msg wire.Message, downChan chan<- struct{})
