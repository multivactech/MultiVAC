...
const OutboundPeerAddr = "127.0.0.1:18555"

func init() {
	peer.SuppressLogForTest()
}
...
func mockRemotePeer() error {
	...
	listener, err := net.Listen("tcp", OutboundPeerAddr)
	if err != nil {
		return err
	}
	go func() {
		...
		p.AssociateConnection(conn,
			wire.UpdatePeerInfoMessage{
				Shards:               []shard.Index{},
				IsStorageNode:        false,
				RPCListenerAddress:   "127.0.0.1:8334",
				LocalListenerAddress: "127.0.0.1:8333",
			})
	}()
...
func Example_newOutboundPeer() {
	...
	p, err := peer.NewOutboundPeer(peerCfg, OutboundPeerAddr)
	if err != nil {
	...
	p.AssociateConnection(conn, wire.UpdatePeerInfoMessage{
		Shards:               []shard.Index{},
		IsStorageNode:        false,
		RPCListenerAddress:   "127.0.0.1:8334",
		LocalListenerAddress: "127.0.0.1:8333",
