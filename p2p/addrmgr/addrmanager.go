...
func (a *AddrManager) savePeers() {
	...
	logger.AddrMgrLogger().Debugf(a.peersFile)
...
func (a *AddrManager) reset() {
	...
	if err != nil {
		logger.AddrMgrLogger().Debugf("failed to read from random source,err:%v", err)
	}
	for i := range a.addrNew {
...
func New(dataDir string, loadSavedPeer bool, lookupFunc func(string) ([]net.IP, error)) *AddrManager {
	am := AddrManager{
		peersFile:      filepath.Join(dataDir, "peers.json"),
		lookupFunc:     lookupFunc,
		loadSavedPeer:  loadSavedPeer,
