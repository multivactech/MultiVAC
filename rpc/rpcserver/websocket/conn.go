...
func (c *Conn) advanceFrame() (int, error) {
	...
	if frameType == continuationFrame || frameType == TextMessage || frameType == BinaryMessage {
		...
		if c.readLimit > 0 && c.readLength > c.readLimit {
			err := c.WriteControl(CloseMessage, FormatCloseMessage(CloseMessageTooBig, ""), time.Now().Add(writeWait))
			if err != nil {
				return noFrame, err
			}
	...
	case CloseMessage:
		err := c.WriteControl(CloseMessage, []byte{}, time.Now().Add(writeWait))
		if err != nil {
			return noFrame, err
		}
...
func (c *Conn) handleProtocolError(message string) error {
	err := c.WriteControl(CloseMessage, FormatCloseMessage(CloseProtocolError, message), time.Now().Add(writeWait))
	if err != nil {
		return err
	}
