...
func TestControl(t *testing.T) {
	...
	for _, isServer := range []bool{true, false} {
		for _, isWriteControl := range []bool{true, false} {
			...
			if isWriteControl {
				err := wc.WriteControl(PongMessage, []byte(message), time.Now().Add(time.Second))
				if err != nil {
				...
				if err != nil {
					t.Errorf("%s: wc.NextReader() returned %v", name, err)
					continue
				}
...
func TestCloseBeforeFinalFrame(t *testing.T) {
	...
	_, err := w.Write(make([]byte, bufSize+bufSize/2))
	if err != nil {
		t.Fatalf("w.Write() returned, %v", err)
	}
	err = wc.WriteControl(CloseMessage, FormatCloseMessage(CloseNormalClosure, ""), time.Now().Add(10*time.Second))
	if err != nil {
...
func TestReadLimit(t *testing.T) {
	...
	w, _ := wc.NextWriter(BinaryMessage)
	_, err := w.Write(message[:readLimit-1])
	if err != nil {
		t.Fatalf("w.Write() returned, %v", err)
	}
	err = wc.WriteControl(PongMessage, []byte("this is a pong"), time.Now().Add(10*time.Second))
	if err != nil {
		t.Fatalf("w.WriteControl() returned, %v", err)
	}
	...
	err = wc.WriteMessage(BinaryMessage, message[:readLimit+1])
	if err != nil {
