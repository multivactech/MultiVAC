...
func TestChainSvrCmds(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "gettxout",
			newCmd: func() (interface{}, error) {
				return btcjson.NewCmd("gettxout", "123")
			},
			staticCmd: func() interface{} {
				return btcjson.NewGetTxOutCmd("123", nil)
			},
			marshalled: `{"jsonrpc":"1.0","method":"gettxout","params":["123"],"id":1}`,
		...
		},
		{
			...
			newCmd: func() (interface{}, error) {
				return btcjson.NewCmd("gettxout", "123", true)
			},
			staticCmd: func() interface{} {
				return btcjson.NewGetTxOutCmd("123", btcjson.Bool(true))
			},
			marshalled: `{"jsonrpc":"1.0","method":"gettxout","params":["123",true],"id":1}`,
			unmarshalled: &btcjson.GetTxOutCmd{
				HexOutPoint:    "123",
				IncludeMempool: btcjson.Bool(true),
			},
