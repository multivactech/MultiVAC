...
// TestAsync is a temporary rpc command to trigger tx generate on storage node side.
// It should be cleaned up once client tx generation is stable.
func (c *Client) TestAsync() FuturePingResult {
	cmd := btcjson.NewTestCmd("10")
	return c.sendCmd(cmd)
}

// SendTest generates some test Txs on storage node and returns error message.
// TODO: test whether this method can still be used at version 3.0.
func (c *Client) SendTest() error {
	return c.TestAsync().Receive()
}

// TestTxAsync is a temporary rpc command to trigger tx generate on storage node side.
// It should be cleaned up once client tx generation is stable.
func (c *Client) TestTxAsync(numOfTx int) FuturePingResult {
	cmd := btcjson.NewTestTxCmd(fmt.Sprintf("%d", numOfTx))
	return c.sendCmd(cmd)
}

// SendTestTx generates some test Txs on storage node and returns error message.
// TODO: test whether this method can still be used at version 3.0.
func (c *Client) SendTestTx(numOfTx int) error {
	return c.TestTxAsync(numOfTx).Receive()
}
...
func (c *Client) SendTransferTx(txDetail string) (*string, error) {
	return c.SendTransferTxAsync(txDetail).Receive()
}

// SendTransferTxAsync returns an instance of a type that can be used to get
// the result of the RPC at some future time by invoking the Receive function on
// the returned instance.
//
...
func (c *Client) SendTransferTxAsync(txDetail string) FutureReceivedResult {
	cmd := btcjson.NewTransferTxCmd(txDetail)
	return c.sendCmd(cmd)
}
...
// the result of sending transfer tx to storage node.
func (r FutureReceivedResult) Receive() (*string, error) {
	...
	// Unmarshal as shardsInfo result objects.
	var result string
...
// FutureSendRawTransaction is a future promise to deliver the result of a RPC
// invocation (or an applicable error).
type FutureSendRawTransaction chan *response

// Receive waits for the response promised by the future and returns the result
// of sending a rawTransaction.
...
func (c *Client) SendRawTransactionAsync(HexTx string) FutureSendRawTransaction {
	cmd := btcjson.NewSendRawTransactionCmd(HexTx)
	return c.sendCmd(cmd)
}

// SendRawTransaction sends a Tx to the storage node in it's shard.
func (c *Client) SendRawTransaction(HexTx string) error {
	return c.SendRawTransactionAsync(HexTx).Receive()
}

// FutureGetTxOut is a future promise to deliver the result of a RPC
// invocation (or an applicable error).
...
func (c *Client) GetTxOutAsync(hexOutPoint string) FutureGetTxOut {
	cmd := btcjson.NewGetTxOutCmd(hexOutPoint, nil)
	return c.sendCmd(cmd)
}

// GetTxOut returns the given outPoint's value and state.
func (c *Client) GetTxOut(hexOutPoint string) (byte, error) {
	return c.GetTxOutAsync(hexOutPoint).Receive()
}

// FutureShardInfoResult is a future promise to deliver the result of a QueryShardInfoAsync RPC
// invocation (or an applicable error).
type FutureShardInfoResult chan *response

// Receive waits for the response promised by the future and returns the result
// of querying peer shard info.
func (r FutureShardInfoResult) Receive() (*btcjson.ShardsInfo, error) {
	res, err := receiveFuture(r)
	if err != nil {
		return nil, err
	}
	...
	// Unmarshal as shardsInfo result objects.
	var shardsInfo btcjson.ShardsInfo
	err = json.Unmarshal(res, &shardsInfo)
	if err != nil {
		return nil, err
	}
...
func (c *Client) QueryShardInfo() (*btcjson.ShardsInfo, error) {
	return c.QueryShardInfoAsync().Receive()
}

// QueryShardInfoAsync returns an instance of a type that can be used to get
// the result of the RPC at some future time by invoking the Receive function on
// the returned instance.
//
// See QueryShardInfo for the blocking version and more details.
func (c *Client) QueryShardInfoAsync() FutureShardInfoResult {
	cmd := btcjson.NewQueryShardInfoCmd()
	return c.sendCmd(cmd)
}

// FutureBlockInfoResult is a future promise to deliver the result of a QueryBlockInfoAsync RPC
// invocation (or an applicable error).
...
func (r FutureBlockInfoResult) Receive() (*btcjson.BlockInfo, error) {
	...
	var blockInfo btcjson.BlockInfo
...
func (c *Client) QueryBlockInfo(startHeight int64, shardID string) (*btcjson.BlockInfo, error) {
	return c.QueryBlockInfoAsync(startHeight, shardID).Receive()
}

// QueryBlockInfoAsync returns an instance of a type that can be used to get
// the result of the RPC at some future time by invoking the Receive function on
// the returned instance.
//
...
func (c *Client) QueryBlockInfoAsync(startHeight int64, shardID string) FutureBlockInfoResult {
	cmd := btcjson.NewQueryBlockCmd(startHeight, shardID)
	return c.sendCmd(cmd)
}

// SetHeartBeatTime is used to reset heart beat params.
// TODO: to test whether it will be used.
func (c *Client) SetHeartBeatTime(online, deviation, interval int64) {
	cmd := btcjson.NewSetHeartBeatTimeCmd(online, deviation, interval)
	c.sendCmd(cmd)
}
...
func (c *Client) QueryBlockSliceInfo(startHeight int64, endHeight int64, shardID string) (*btcjson.BlockInfo, error) {
	return c.QueryBlockSliceInfoAsync(startHeight, endHeight, shardID).Receive()
}

// QueryBlockSliceInfoAsync returns an instance of a type that can be used to get
// the result of the RPC at some future time by invoking the Receive function on
// the returned instance.
//
...
func (c *Client) QueryBlockSliceInfoAsync(startHeight int64, endHeight int64, shardID string) FutureBlockInfoResult {
	cmd := btcjson.NewQueryBlockSliceCmd(startHeight, endHeight, shardID)
	return c.sendCmd(cmd)
}
...
// SendPrepareForStop ...
// TODO: see whether it will be used.
func (c *Client) SendPrepareForStop() {
	c.sendCmd(btcjson.NewPrepareForStopCmd())
}
...
func (c *Client) QuerySmartContractInfo(contractAddress multivacaddress.Address, shardIdx shard.Index) (*btcjson.BlockInfo, error) {
	return c.QuerySmartContractInfoAsync(contractAddress, shardIdx).Receive()
}

// QuerySmartContractInfoAsync returns an instance of a type that can be used to get the
// result of the RPC at some future time by invoking the Receive function on the
// returned instance.
//
...
func (c *Client) QuerySmartContractInfoAsync(contractAddress multivacaddress.Address, shardIdx shard.Index) FutureBlockInfoResult {
	cmd := btcjson.NewQuerySmartContractInfoCmd(contractAddress, shardIdx)
	return c.sendCmd(cmd)
}
