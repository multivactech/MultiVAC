...
var (
	// ErrEndOfLine is returned when the end of the current list
	// has been reached during streaming.
	ErrEndOfLine = errors.New("rlp: end of list")

	// ErrExpectedString return expected String or Byte error.
	ErrExpectedString = errors.New("rlp: expected String or Byte")
	// ErrExpectedList returns expected List error.
	ErrExpectedList = errors.New("rlp: expected List")
	// ErrCanonInt returns non-canonical integer format error.
	ErrCanonInt = errors.New("rlp: non-canonical integer format")
	// ErrCanonSize returns non-canonical size information error.
	ErrCanonSize = errors.New("rlp: non-canonical size information")
	// ErrElemTooLarge returns element is to large error.
	ErrElemTooLarge = errors.New("rlp: element is larger than containing list")
	// ErrValueTooLarge returns input length is to longer than available error.
	ErrValueTooLarge = errors.New("rlp: value size exceeds available input length")
	// ErrMoreThanOneValue returns input more than one value error.
	ErrMoreThanOneValue = errors.New("rlp: input contains more than one value")

	// Internal errors.
	errNotInList     = errors.New("rlp: call of ListEnd outside of any list")
	errNotAtEOL      = errors.New("rlp: call of ListEnd not positioned at ErrEndOfLine")
...
func decodeSliceElems(s *Stream, val reflect.Value, elemdec decoder) error {
	...
	for ; ; i++ {
		...
		// decode into element
		if err := elemdec(s, val.Index(i)); err == ErrEndOfLine {
			break
		} else if err != nil {
...
const (
	// Byte defined to enum a new kind.
	Byte Kind = iota
	// String defined to enum a new kind.
	String
	// List defined to enum a new kind.
	List
)
...
// length of the list, all operations will return error ErrEndOfLine.
...
func (s *Stream) int(maxbits int) (int64, error) {
	...
	switch kind {
	case Byte:
		// TODO(liboshen): should always return error.
		if s.byteval == 0 {
			return 0, ErrCanonInt
		}
		s.kind = -1 // Rearm Kind.
...
// end has been reached, any Stream operation will return ErrEndOfLine.
...
func (s *Stream) readInt(size byte) (int64, error) {
	...
	default:
		...
		abs := int64(binary.BigEndian.Uint64(s.uintbuf))
		if sign == 0x01 {
			return abs, nil
		} else if sign == 0x80 {
			return -abs, nil
