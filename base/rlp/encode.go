...
var (
	// EmptyString is Common encoded values.It will be used when implementing EncodeRLP.
	EmptyString = []byte{0x80}
	// EmptyList is Common encoded values.It will be used when implementing EncodeRLP.
	EmptyList = []byte{0xC0}
)
...
// TODO(libo): use more compact encoding.
func writeInt(val reflect.Value, w *encbuf) error {
	...
	if i == 0 {
		...
		s := putint(w.sizebuf[2:], abs)
		w.sizebuf[0] = 0x80 + 1 + byte(s)
		if sign {
			w.sizebuf[1] = 0x01
		} else {
			w.sizebuf[1] = 0x80
		}
		w.str = append(w.str, w.sizebuf[:s+2]...)
	}
	return nil
}
